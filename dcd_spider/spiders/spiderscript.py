...
class DiseaseSpider(scrapy.Spider):
    ...
    def parse_disease(self, response):        
        name = response.css('header h1 a::text').get(default='').strip()
        
        # letter = response.
                
        description =  response.css('div.content').xpath('.//div[h2/text() = "Overview"]/p[1]/text()').get()
        
        symptoms = response.css('div.content').xpath('.//div[h2/text() = "Symptoms"]/ul[1]/li/text()').getall()
        
        causes =  response.css('div.content').xpath('.//h2[text() = "Causes"]/following-sibling::ul[1]/li/strong/text()').getall()
        
        risks = response.css('div.content').xpath('.//h2[text() = "Risk factors"]/following-sibling::ul[1]/li/strong/text()').getall()
        
        complications = response.css('div.content').xpath('.//h2[text() = "Complications"]/following-sibling::ul[1]/li/text()').getall()
        
        preventions = response.css('div.content').xpath('.//h2[text() = "Prevention"]/following-sibling::ul[1]/li/strong/text()').getall()
