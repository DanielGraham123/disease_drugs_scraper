...
def search(request):
    ...
    if query:
        ...
        drugs_result = Drug.objects.filter(Q(name__icontains=query) | Q(uses__icontains=query) | Q(generic_name__icontains=query) | Q(side_effects__icontains=query) | Q(precautions__icontains=query) | Q(interactions__icontains=query))
    else:
